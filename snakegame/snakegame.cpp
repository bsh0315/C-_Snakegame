...
int gameDuration = 300; // Game duration in seconds
...
void SaveGameState(const string& filename, const vector<Player>& players, int remainingTime, int dfc) {
...
void LoadGameState(const string& filename, vector<Player>& players, int& remainingTime, int& dfc) {
...
int main() {
    ...
    case 3: {
        LoadGameState(filename, players, gameDuration, dfc);
        break;
    }
    case 4:
        return 0;
    ...
    while (true) {
        ...
        if (saveAndQuit) {
            SaveGameState(filename, players, remaining_time, dfc);
            break;
        }
